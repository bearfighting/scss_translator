/* This file was generated by SableCC (http://www.sablecc.org/). */

package scss.node;

import scss.analysis.*;

@SuppressWarnings("nls")
public final class ANumberUnitExpression extends PExpression
{
    private TNumberLiteral _numberLiteral_;
    private TUnit _unit_;

    public ANumberUnitExpression()
    {
        // Constructor
    }

    public ANumberUnitExpression(
        @SuppressWarnings("hiding") TNumberLiteral _numberLiteral_,
        @SuppressWarnings("hiding") TUnit _unit_)
    {
        // Constructor
        setNumberLiteral(_numberLiteral_);

        setUnit(_unit_);

    }

    @Override
    public Object clone()
    {
        return new ANumberUnitExpression(
            cloneNode(this._numberLiteral_),
            cloneNode(this._unit_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseANumberUnitExpression(this);
    }

    public TNumberLiteral getNumberLiteral()
    {
        return this._numberLiteral_;
    }

    public void setNumberLiteral(TNumberLiteral node)
    {
        if(this._numberLiteral_ != null)
        {
            this._numberLiteral_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._numberLiteral_ = node;
    }

    public TUnit getUnit()
    {
        return this._unit_;
    }

    public void setUnit(TUnit node)
    {
        if(this._unit_ != null)
        {
            this._unit_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._unit_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._numberLiteral_)
            + toString(this._unit_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._numberLiteral_ == child)
        {
            this._numberLiteral_ = null;
            return;
        }

        if(this._unit_ == child)
        {
            this._unit_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._numberLiteral_ == oldChild)
        {
            setNumberLiteral((TNumberLiteral) newChild);
            return;
        }

        if(this._unit_ == oldChild)
        {
            setUnit((TUnit) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
